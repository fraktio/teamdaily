  ...
  return (
    <div>
      ...
      <div className="matrix-data-container">
        <Link to="/index" className="btn btn-info btn-back">Takaisin etusivulle</Link>
              ...
              {projects.sort((a, b) => alphabeticalSort(a.name,b.name)).map(project => {
                ...
                return (
                  <th key={project.id} className="fixed-width wide clickable">
                    {project.name}<span className="counter-ball">{employees.size}</span>
                  </th>
                );
              })}
              </tr>
            ...
            {employees.map(employee =>
              <tr key={employee.name + '-row'}>
                ...
                {projects.sort((a, b) => alphabeticalSort(a.name,b.name)).map((project) => {
                  ...
                  return (
                    <td className={employeeProject ? 'green' : ''} key={employee.name + '-' + project.id + '-cell'}> </td>
                  );
                })}
              </tr>
            )}
